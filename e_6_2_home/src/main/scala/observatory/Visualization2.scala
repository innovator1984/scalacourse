...
object Visualization2 {
  ...
  /**
    ...
    */
  def myPixelLocations(offsetX: Int, offsetY: Int, zoom: Int, imageWidth: Int, imageHeight: Int):immutable.IndexedSeq[(Int, Location)] = {
    for{
      xPixel <- 0 until imageWidth
      yPixel <- 0 until imageHeight
    } yield xPixel + yPixel * imageWidth -> MyTile(xPixel.toDouble / imageWidth + offsetX, yPixel.toDouble / imageHeight + offsetY, zoom).location
  }
  ...
  def visualizeGrid(
    ...
    val pixels = myPixelLocations(x, y, zoom, imageWidth, imageHeight).par.map{
